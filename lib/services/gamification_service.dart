...
class GamificationService extends ChangeNotifier {
  ...
  Future<void> _loadBadges() async {
    try {
      if (_currentUserId == null) {
        debugPrint('GamificationService._loadBadges: Pas d\'utilisateur connecté');
      ...
      if (response.isEmpty) {
        debugPrint('GamificationService._loadBadges: Aucun badge trouvé dans la base de données');
        _badges = _getDefaultBadges();
        notifyListeners();
        return;
      }

      debugPrint('GamificationService._loadBadges: ${response.length} badges récupérés depuis la base de données');
      ...
      debugPrint('GamificationService._loadBadges: ${_badges.length} badges chargés avec succès');
      notifyListeners();
    } catch (e, stackTrace) {
  ...
  void onOrderPlaced(double orderValue) {
    ...
    unawaited(addPoints(points, 'Commande passée',
  ...
  void onReviewLeft() {
    unawaited(addPoints(10, 'Avis laissé',
  ...
  void onAppShared() {
    unawaited(addPoints(25, 'Application partagée',
  ...
  void onNewDishTried() {
    unawaited(addPoints(15, 'Nouveau plat essayé',
