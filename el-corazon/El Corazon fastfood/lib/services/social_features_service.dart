...
class SocialFeaturesService extends ChangeNotifier {
  ...
  /// Initialise le service de fonctionnalités sociales
  Future<void> initialize() async {
    if (_isInitialized) return;

    try {
      // Charger les données sociales depuis la base de données
  ...
  Future<void> _loadSocialData() async {
    try {
      ...
      // Charger les événements sociaux (si table existe)
      // await _loadSocialEvents();

      debugPrint(
        '✅ Données sociales chargées: ${_groups.length} groupes, ${_posts.length} posts',
      );
    } catch (e) {
      debugPrint('❌ Erreur lors du chargement des données sociales: $e');
    }
  }

  /// Charge les groupes sociaux depuis la base de données
  Future<void> _loadSocialGroups() async {
    try {
      ...
      debugPrint('✅ ${_groups.length} groupes sociaux chargés');
    } catch (e) {
      debugPrint('❌ Erreur lors du chargement des groupes: $e');
    }
  }

  /// Charge les posts sociaux depuis la base de données
  Future<void> _loadSocialPosts() async {
    try {
      ...
      for (final item in response as List) {
        ...
        final comments = (commentsResponse as List).map((c) {
      ...
      debugPrint('✅ ${_posts.length} posts sociaux chargés');
    } catch (e) {
      debugPrint('❌ Erreur lors du chargement des posts: $e');
    }
  }

  /// Parse le type de groupe depuis la base de données
  ...
  /// Parse le type de post depuis la base de données
