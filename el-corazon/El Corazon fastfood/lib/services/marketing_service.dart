...
class MarketingService extends ChangeNotifier {
  ...
  Future<PredictiveAnalytics> generateSalesForecast({
    ...
    final Map<String, dynamic> analysis =
        _analyzeHistoricalSales(historicalOrders);
  ...
  Future<PredictiveAnalytics> predictInventoryNeeds({
    ...
    for (final item in menuItems) {
      ...
      final double dailyAverage =
          recentCount / 7.0; // Assuming 7 days of recent data
        ...
        orders.take(3).every(
              (order) => orders.first.items.any(
                (firstItem) => order.items
                    .any((orderItem) => orderItem.name == firstItem.name),
              ),
            );
  ...
  List<String> _generateRecommendedActions(
    ...
    final orderFreq = behaviorPatterns['orderFrequency'] as num?;
    if (orderFreq != null && orderFreq < 1.0) {
      actions.add('Envoyer rappel hebdomadaire personnalisé');
      actions.add('Proposer un menu découverte');
    }

    // High value customer actions
    final avgOrderValue = preferences['averageOrderValue'] as num?;
    if (avgOrderValue != null && avgOrderValue > 10000) {
  ...
  Future<bool> sendTargetedNotification({
    ...
    try {
      ...
      final metricsUpdate = {
        'sent': ((currentCampaign.metrics['sent'] as num?)?.toInt() ?? 0) +
            userIds.length,
      ...
      try {
        final int campaignIndex =
            _campaigns.indexWhere((c) => c.id == campaignId);
        if (campaignIndex != -1) {
          final campaign = _campaigns[campaignIndex];
          final Map<String, dynamic> newMetrics = Map.from(campaign.metrics);
          newMetrics['sent'] =
              ((newMetrics['sent'] as num?)?.toInt() ?? 0) + userIds.length;
  ...
  Map<String, dynamic> getMarketingDashboard() {
    ...
    final num totalSent = _campaigns.fold(
      0,
      (sum, c) => sum + ((c.metrics['sent'] as num?)?.toInt() ?? 0),
    );

    final num totalClicks = _campaigns.fold(
      0,
      (sum, c) => sum + ((c.metrics['clicks'] as num?)?.toInt() ?? 0),
    );
    ...
    return {
      ...
      'highChurnRiskCustomers': _customerInsights.values
          ...
          .length,
      'loyalCustomers': _customerInsights.values.where((insight) {
        final loyaltyScore = (insight.behaviorPatterns['loyaltyScore'] as num?);
        return loyaltyScore != null && loyaltyScore.toDouble() > 0.7;
      }).length,
  ...
  Map<String, dynamic> getCampaignPerformance(String campaignId) {
    try {
      ...
      final int sent = (campaign.metrics['sent'] as num?)?.toInt() ?? 0;
      final int clicks = (campaign.metrics['clicks'] as num?)?.toInt() ?? 0;
      final int conversions =
          (campaign.metrics['conversions'] as num?)?.toInt() ?? 0;
      final int views = (campaign.metrics['views'] as num?)?.toInt() ?? 0;
  ...
  Future<void> recordCampaignView(String campaignId) async {
    try {
      ...
      final currentCampaign = _campaigns[campaignIndex];
      final metricsUpdate = {
        'views': ((currentCampaign.metrics['views'] as num?)?.toInt() ?? 0) + 1,
  ...
  Future<void> recordCampaignClick(String campaignId) async {
    try {
      ...
      final metricsUpdate = {
        'clicks':
            ((currentCampaign.metrics['clicks'] as num?)?.toInt() ?? 0) + 1,
  ...
  Future<void> recordCampaignConversion(String campaignId) async {
    try {
      ...
      final metricsUpdate = {
        'conversions':
            ((currentCampaign.metrics['conversions'] as num?)?.toInt() ?? 0) +
                1,
