/// Fondations partagées des 3 apps Flutter d'El Corazón (Phase 6, tranche 1) :
/// client API, authentification et session. Voir
/// `docs/architecture/04-migration-flutter.md`.
library;

export 'src/accounts/administration_repository.dart';
export 'src/accounts/admin_role.dart';
export 'src/accounts/customer.dart';
export 'src/accounts/customer_stats.dart';
export 'src/analytics/analytics_repository.dart';
export 'src/analytics/report.dart';
export 'src/analytics/reporting_repository.dart';
export 'src/auth/auth_repository.dart';
export 'src/auth/session.dart';
export 'src/auth/token_storage.dart';
export 'src/auth/verification_challenge.dart';
export 'src/calls/call.dart';
export 'src/calls/call_repository.dart';
export 'src/cart/cart.dart';
export 'src/cart/cart_repository.dart';
export 'src/catalog/catalog_repository.dart';
export 'src/catalog/category.dart';
export 'src/catalog/indisponibilite.dart';
export 'src/catalog/managed_catalog_repository.dart';
export 'src/catalog/managed_category.dart';
export 'src/catalog/managed_menu_item.dart';
export 'src/catalog/managed_review.dart';
export 'src/catalog/menu_item.dart';
export 'src/catalog/option_template.dart';
export 'src/catalog/review.dart';
export 'src/delivery/assignment.dart';
export 'src/delivery/assignment_offer.dart';
export 'src/delivery/courier_application.dart';
export 'src/delivery/courier_profile.dart';
export 'src/delivery/courier_shift.dart';
export 'src/delivery/delivery_repository.dart';
export 'src/delivery/earnings.dart';
export 'src/delivery/managed_assignment_repository.dart';
export 'src/delivery/managed_courier_repository.dart';
export 'src/design/emojis/app_emoji.dart';
export 'src/design/emojis/app_emojis.dart';
export 'src/diagnostics/journal.dart';
export 'src/directions/directions_repository.dart';
export 'src/directions/geo_point.dart';
export 'src/directions/route_info.dart';
export 'src/directions/route_step.dart';
export 'src/geography/city.dart';
export 'src/geography/delivery_zone.dart';
export 'src/geography/zone_shape.dart';
export 'src/geography/managed_city.dart';
export 'src/geography/managed_country.dart';
export 'src/geography/managed_geography_repository.dart';
export 'src/geography/places.dart';
export 'src/geography/reverse_geocode.dart';
export 'src/groupcarts/group_cart.dart';
export 'src/groupcarts/group_cart_repository.dart';
export 'src/inventory/inventory.dart';
export 'src/inventory/managed_inventory_repository.dart';
export 'src/inventory/managed_recipe_repository.dart';
export 'src/inventory/quantity.dart';
export 'src/geography/delivery_check.dart';
export 'src/geography/geography_reference.dart';
export 'src/geography/geography_repository.dart';
export 'src/geography/zone_resolution.dart';
export 'src/gamification/achievement.dart';
export 'src/gamification/badge.dart';
export 'src/gamification/challenge.dart';
export 'src/gamification/gamification_repository.dart';
export 'src/gamification/managed_gamification_repository.dart';
export 'src/gamification/vocabulaire_gamification.dart';
export 'src/location/location_availability.dart';
export 'src/location/position_simulee.dart';
export 'src/loyalty/loyalty_repository.dart';
export 'src/loyalty/points_account.dart';
export 'src/loyalty/points_entry.dart';
export 'src/loyalty/reward.dart';
export 'src/loyalty/reward_redemption.dart';
export 'src/loyalty/subscription.dart';
export 'src/loyalty/vocabulaire_fidelite.dart';
export 'src/navigation/etat_navigation.dart';
export 'src/navigation/geo_calcul.dart';
export 'src/navigation/moteur_de_navigation.dart';
export 'src/navigation/phrases_navigation.dart';
export 'src/navigation/suivi_de_trace.dart';
export 'src/models/internal_note.dart';
export 'src/models/money.dart';
export 'src/models/user.dart';
export 'src/network/api_client.dart';
export 'src/notifications/app_notification.dart';
export 'src/notifications/campaign.dart';
export 'src/notifications/campaign_repository.dart';
export 'src/notifications/notification_repository.dart';
export 'src/network/api_exception.dart';
export 'src/network/api_failure.dart';
export 'src/network/message_erreur.dart';
export 'src/network/page.dart';
export 'src/orders/kitchen_order.dart';
export 'src/orders/managed_order_repository.dart';
export 'src/orders/order.dart';
export 'src/orders/order_quote.dart';
export 'src/orders/order_repository.dart';
export 'src/orders/order_statistics.dart';
export 'src/payments/managed_payouts.dart';
export 'src/payments/payment_repository.dart';
export 'src/payments/split_payment.dart';
export 'src/payments/transaction.dart';
export 'src/payments/vocabulaire_paiement.dart';
export 'src/profile/address.dart';
export 'src/promotions/promotion.dart';
export 'src/promotions/promotion_repository.dart';
export 'src/profile/address_repository.dart';
export 'src/profile/customer_preferences.dart';
export 'src/profile/preferences_repository.dart';
export 'src/realtime/chat_message.dart';
export 'src/realtime/realtime_channel.dart';
export 'src/realtime/realtime_event.dart';
export 'src/restaurants/audit_journal.dart';
export 'src/restaurants/kitchen_closure.dart';
export 'src/restaurants/managed_opening_hours_repository.dart';
export 'src/restaurants/restaurant_zone_repository.dart';
export 'src/restaurants/managed_restaurant.dart';
export 'src/restaurants/managed_restaurant_repository.dart';
export 'src/restaurants/restaurant_lifecycle.dart';
export 'src/restaurants/opening_hours.dart';
export 'src/restaurants/restaurant_directory_repository.dart';
export 'src/restaurants/restaurant.dart';
export 'src/search/search_repository.dart';
export 'src/social/post.dart';
export 'src/social/social_group.dart';
export 'src/social/social_repository.dart';
export 'src/support/complaint.dart';
export 'src/support/managed_support.dart';
export 'src/support/return_request.dart';
export 'src/support/support_repository.dart';
export 'src/support/support_ticket.dart';
export 'src/tracking/location_ping.dart';
export 'src/tracking/position_freshness.dart';
export 'src/tracking/tracking_repository.dart';
export 'src/tracking/tracking_settings.dart';
