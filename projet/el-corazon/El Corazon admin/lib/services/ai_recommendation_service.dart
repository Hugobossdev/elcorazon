...
class AIRecommendationService extends ChangeNotifier {
  ...
  Future<void> _generateRecommendations(String userId) async {
    ...
    final historyRecommendations = _getHistoryBasedRecommendations(
      userId,
      menuItems,
    );
    ...
    final timeRecommendations = _getTimeBasedRecommendations(
      preferences,
      menuItems,
    );
    ...
    final uniqueRecommendations = _deduplicateAndRank(
      recommendations,
      preferences,
    );
  ...
  List<MenuItem> _getHistoryBasedRecommendations(
    ...
    for (final category in sortedCategories.take(3)) {
      final categoryItems = menuItems
          .where((item) => item/* .categoryName - REMOVED */.name.toLowerCase() == category.key)
          .toList();
  ...
  List<MenuItem> _getTimeBasedRecommendations(
    ...
    if (hour >= 6 && hour < 11) {
      ...
      final breakfastItems = menuItems
          .where(
            (item) =>
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('breakfast') ||
      ...
      final lunchItems = menuItems
          .where(
            (item) =>
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('lunch') ||
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('burger') ||
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('sandwich'),
          )
          .toList();
      ...
      final dinnerItems = menuItems
          .where(
            (item) =>
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('pizza') ||
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('pasta') ||
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('dinner'),
          )
          .toList();
    ...
    if (isHot) {
      ...
      final coldItems = menuItems
          .where(
            (item) =>
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('drink') ||
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('salad') ||
      ...
      final hotItems = menuItems
          .where(
            (item) =>
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('soup') ||
                item.name.toLowerCase().contains('hot') ||
                item.name.toLowerCase().contains('warm') ||
                item/* .categoryName - REMOVED */.name.toLowerCase().contains('coffee'),
          )
          .toList();
  ...
  double _calculateRecommendationScore(
    ...
    if (preferences.favoriteCategories.contains(
    ...
    if (item.basePrice <= 10 && preferences.priceRange == PriceRange.low) {
      score += 0.2;
    } else if (item.basePrice > 10 &&
        item.basePrice <= 20 &&
        preferences.priceRange == PriceRange.medium) {
      score += 0.2;
    } else if (item.basePrice > 20 && preferences.priceRange == PriceRange.high) {
      score += 0.2;
    }
  ...
  /// Note: Cette méthode devrait charger les items depuis la base de données
  /// Pour l'instant, retourne une liste vide car les items doivent être chargés depuis AppService
  Future<List<MenuItem>> _getAvailableMenuItems() async {
    // Cette méthode devrait charger depuis Supabase ou AppService
    // Pour l'instant, retourner une liste vide
    // Les items seront chargés depuis AppService dans la méthode getRecommendations
    return [];
  }
  ...
  List<MenuItem> getSimilarItems(MenuItem item) {
    ...
    final sameCategoryItems = allItems
        .where(
          (otherItem) =>
              otherItem.categoryId == item.categoryId && otherItem.id != item.id,
        )
        .toList();
  ...
  Map<String, dynamic> analyzeOrderTrends(String userId) {
    ...
    for (final order in orders) {
      for (final item in order.items) {
        categoryTrends[item.categoryId] =
            (categoryTrends[item.categoryId] ?? 0) + 1;
      }
    }
