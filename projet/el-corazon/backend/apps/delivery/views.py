...
from common.permissions import HasPermission, IsCourier, authenticated_user
...
class StaffCourierViewSet(ReadOnlyModelViewSet[CourierProfile]):
    """Flotte, vue par le personnel de ses établissements."""
    ...
    @action(
        detail=True,
        methods=["post"],
        permission_classes=[HasPermission.of("couriers.approve")],
    )
    def verification(self, request: Request, pk: str) -> Response:
        """Valide, rejette ou suspend un dossier.

        Une seule permission couvre les trois : `couriers.approve`. Séparer la
        suspension demanderait `couriers.suspend` — elle existe au registre, et
        l'y brancher est une ligne le jour où l'exploitation le demande.
        """
