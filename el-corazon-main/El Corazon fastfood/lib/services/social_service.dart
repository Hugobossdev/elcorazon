...
class SocialService extends ChangeNotifier {
  ...
  Future<void> _notifyGroupMembers(String groupId, String message, {String? title}) async {
    try {
      // Récupérer tous les membres actifs du groupe
      final groupMembersResponse = await _databaseService.supabase
          .from('group_members')
          .select('user_id')
          .eq('group_id', groupId)
          .eq('is_active', true);
      ...
      final membersList = List<dynamic>.from(groupMembersResponse);
      for (final member in membersList) {
        final memberData = Map<String, dynamic>.from(member as Map);
      ...
      for (final userId in memberUserIds) {
        try {
          await _notificationService.sendSocialNotification(
            userId: userId,
            title: title ?? '👥 Notification de groupe',
            message: message,
          );
        } catch (e) {
          debugPrint('⚠️ Erreur envoi notification à $userId: $e');
        }
      }

      debugPrint('✅ Notifications envoyées aux membres du groupe $groupId');
    } catch (e) {
      debugPrint('❌ Erreur lors de l\'envoi de notifications au groupe: $e');
    }
  }
